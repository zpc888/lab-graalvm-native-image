...
public class DemoAImpl implements ReflectDemo {
    @Override
    public void invoke(List<String> collector) {
        collector.add(String.format(">> %s is called", getClass().getSimpleName()));
        collector.add("load resource: " + loadResource("/reflect-A-resource.txt"));
        collector.add("local class ref: " + (new DemoARef1()));
